        ...
        {displayTrades.length === 0 ? (
          ...
          displayTrades.slice(0, 5).map((trade) => {
            ...
            return (
              <div 
                key={trade.id} 
                className="flex items-center justify-between text-sm py-2 px-2 rounded hover:bg-gray-700/50 transition-colors"
              ...
              >
                ...
                <div className="flex items-center space-x-3 text-right">
                  ...
                  <span 
                    className={`font-medium ${isProfitable ? 'text-green-400' : 'text-red-400'}`}
                    data-testid={`trade-pnl-${trade.id}`}
                  >
                    {isProfitable ? '+' : ''}{formatCurrency(trade.pnl)}
                  </span>
                  <span className="text-xs text-gray-500 w-12" data-testid={`trade-time-${trade.id}`}>
