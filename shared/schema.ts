...
// Phase A - Stevie External Connectors & Schemas (Updated for compatibility)
export const marketBars = pgTable("market_bars", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  symbol: varchar("symbol").notNull(),
  timeframe: varchar("timeframe").notNull(), // '1m', '5m', '1h', '1d'
  timestamp: timestamp("timestamp").notNull(),
  open: varchar("open").notNull(),
  high: varchar("high").notNull(),
  low: varchar("low").notNull(),
  close: varchar("close").notNull(),
  ...
  index("idx_market_bars_symbol_timestamp").on(table.symbol, table.timestamp),
  index("idx_market_bars_provider").on(table.provider),
]);
...
export const orderbookSnaps = pgTable("orderbook_snaps", {
  ...
  symbol: varchar("symbol").notNull(),
  timestamp: timestamp("timestamp").notNull(),
  bid: varchar("bid").notNull(),
  ask: varchar("ask").notNull(),
  spreadBps: real("spread_bps").notNull(),
  depth1bp: varchar("depth_1bp"),
  depth5bp: varchar("depth_5bp"),
...
export const sentimentTicks = pgTable("sentiment_ticks", {
  ...
  volume: integer("volume"), // mentions/posts count
  topic: varchar("topic"),
...
export const onchainTicks = pgTable("onchain_ticks", {
  ...
  metric: varchar("metric").notNull(), // 'gas_price', 'whale_transfers', 'hashrate'
...
export const macroEvents = pgTable("macro_events", {
  ...
  windowBeforeMs: integer("window_before_ms").default(3600000), // 1 hour
  windowAfterMs: integer("window_after_ms").default(3600000), // 1 hour
  provider: varchar("provider").notNull(), // 'trading_economics'
  fetchedAt: timestamp("fetched_at").defaultNow(),
  provenance: jsonb("provenance").notNull(),
}, (table) => [
...
export const connectorHealth = pgTable("connector_health", {
  ...
  quotaCost: integer("quota_cost").default(0),
  lastChecked: timestamp("last_checked").defaultNow(),
}, (table) => [
  index("idx_connector_health_provider").on(table.provider),
]);
...
// Phase A Type Exports for New Tables
