...
// Coarse grid generator
function* grid(space: SearchSpace): Generator<Record<string, number>> {
  // WARNING: Full grid is huge; we'll subsample randomly for the coarse pass.
  // We'll instead draw N random points within bounds.
  const N = Number(process.env.COARSE_N || 120); // adjust
  const keys = Object.keys(space);
  for (let n = 0; n < N; n++) {
...
async function main() {
  ...
  for (const params of grid(SPACE)) {
    try {
      ...
      const score = cashGrowthScore(metrics);
      const row = {
        phase: "coarse", score, reasons: reasons.join("|"), ...params,
        pf: metrics?.headline?.profitFactor, mdd: metrics?.headline?.maxDrawdownPct,
        sharpe: metrics?.headline?.sharpe, tradesPerDay: metrics?.tradesPerDay
      };
      rows.push(row);
      if (!reasons.length && (!best || score > best.score)) best = row;
      console.log("coarse:", row);
    } catch (e: any) {
      console.error("coarse_fail:", e.message);
    }
  ...
  if (!best) throw new Error("No feasible config found in coarse pass");

  console.log("✅ Best coarse config found:", best);

  // Refine region around best with Optuna (Python runner)
  const studyDB = process.env.OPTUNA_RDB || "sqlite:///artifacts/tuning/stevie_optuna.db";
  const envVars = Object.fromEntries(
    Object.entries(best).map(([k, v]) => [`BEST_${k}`, String(v)])
  );
  
  console.log("🔄 Starting Optuna refinement...");
  const py = run("python", ["tools/tune/stevie_optuna.py"], { 
    OPTUNA_RDB: studyDB, 
    ...envVars 
  });
  
  if (!py.ok) throw new Error(py.out);
  console.log(py.out);
